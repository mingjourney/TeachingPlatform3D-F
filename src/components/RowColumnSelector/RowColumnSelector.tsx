...
const RowColumnSelector: React.FC<Props> = ({ row, column, emitResult }) => {
  ...
  const handleCellHover = (rowIndex: number, columnIndex: number) => {
    if (rowIndex === -1 && column === -1) {
      setHoveredCell({ row: clickedCell?.row, column: clickedCell?.column })
      return
    }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center gap-2 ">
        <div className="border border-gray-400 text-xs w-12 h-5 text-center leading-5">
          讲台
        </div>
